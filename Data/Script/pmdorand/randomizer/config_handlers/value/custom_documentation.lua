...
return setter.builder() --[[@as ConfigSetterBuilder<Config.CustomDocumentation>]]
    ...
    :with_select(function(entry)
        ...
        if #entry.texts > 1 then
            entry.texts[2][1] = displays:get(entry.setting.__title).display(this, entry.value, entry)
            entry:update_text()
        end
        return res
    end)
